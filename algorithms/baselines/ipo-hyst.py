    ...
    for epoch in range(epochs):
        ...
        for _ in range(config["learning_iters"]):
            for (
                ...
                ratio_cliped = torch.clamp(ratio, 1 - config["epsilon"], 1 + config["epsilon"])
                if ((optim_case == 1) and (ep_costs < args.cost_limit)) or \
                   ((optim_case == 0) and (ep_costs < config["hyst_frac"]*args.cost_limit)):
                    optim_case = 1
                    loss_pi = -torch.min(ratio * adv_r_b, ratio_cliped * adv_r_b).mean()
                    loss_pi_c = (ratio * adv_c_b).mean()
                    loss_pi_c /= config["t"]*(ep_costs - arcs.cost_limit)
                    loss_pi += loss_pi_c
                else:  # recovery
                    optim_case = 0
                    loss_pi = torch.max(ratio * adv_c_b, ratio_cliped * adv_c_b).mean()
