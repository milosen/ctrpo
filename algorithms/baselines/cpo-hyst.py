...
def main(args, cfg_env=None):
    ...
    for epoch in range(epochs):
        ...
        cost_hys = logger.get_stats("Metrics/EpCost") - args.cost_limit*config["hyst_frac"]
        ...
        if ((not (optim_case == 0)) and (ep_costs < 0 and b_grads.dot(b_grads) <= 1e-6)) or \
           ((optim_case == 0) and (cost_hys < 0 and b_grads.dot(b_grads) <= 1e-6)):
            optim_case = 4
            A = torch.zeros(1)
            B = torch.zeros(1)
        else:
            ...
            if ((not (optim_case == 0)) and (ep_costs < 0 and B < 0)) or \
               ((optim_case == 0) and (cost_hys < 0 and B < 0)):
                optim_case = 3
            elif ((not (optim_case == 0)) and (ep_costs < 0 <= B)) or \
                 ((optim_case == 0) and (cost_hys < 0 <= B)):
                optim_case = 2
            elif ep_costs >= 0 and B >= 0 and not (optim_case == 0):
                optim_case = 1
