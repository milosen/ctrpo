...
default_cfg = {
    ...
    "scheduler": "constant",  # constant, linear, step
}
    ...
    for epoch in range(epochs):
        ...
        for _ in range(config["learning_iters"]):
            for (
                ...
                # cost loss
                b = (1-config["gamma"])*args.cost_limit-ep_costs
                Ac = torch.max(ratio * adv_c_b, ratio_clipped * adv_c_b).mean()
                loss_pi_c = torch.nn.functional.softplus(Ac - b, beta=config["beta"])

                loss_pi = loss_pi_r + loss_pi_c + config["entropy"]*policy.actor(obs_b).entropy().mean()
