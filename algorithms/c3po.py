...
from baselines.common.buffer import VectorizedOnPolicyBuffer
from baselines.common.env import make_sa_mujoco_env, make_sa_isaac_env
from baselines.common.lagrange import Lagrange
...
default_cfg = {
    ...
    "w": 0.05,
    "scheduler": "linear",  # constant, linear, step
}
...
def main(args, cfg_env=None):
    ...
    kappa_start_factor = 0.1
    kappa_end_factor = 1.

    def barrier_div(pi_old, pi_new, data, ep_costs):
        kl = torch.distributions.kl.kl_divergence(pi_old, pi_new).sum(-1, keepdim=True).mean()
        b = args.cost_limit - ep_costs
        ...
        if b<=0:  # recovery
            ...
            alpha = 1/(1-config["gamma"])*((torch.exp(log_prob - data["log_prob"]) - 1)*data['adv_c']).mean()
            ...
            return kl + ((b-alpha)/b - torch.log((b-alpha)/b) - 1)
    
    # training loop
    for epoch in range(epochs):
        rollout_start_time = time.time()
        if config["scheduler"] == "linear":
            kappa = config["kappa"]*(kappa_start_factor + (kappa_end_factor-kappa_start_factor)*epoch/epochs)
        elif config["scheduler"] == "constant":
            kappa = config["kappa"]
        elif config["scheduler"] == "step":
            if epoch < epochs/2:
                kappa = config["kappa"]*kappa_start_factor
            else:
                kappa = config["kappa"]*kappa_start_factor
        else:
        ...
        for _ in range(config["learning_iters"]):
            for (
                ...
                ratio_clipped = torch.clamp(ratio, 1 - config["epsilon"], 1 + config["epsilon"])

                # reward loss
                loss_pi_r = -torch.min(ratio * adv_r_b, ratio_clipped * adv_r_b).mean()

                # cost loss
                Ac = torch.max(ratio * adv_c_b, ratio_clipped * adv_c_b).mean()

                # cost loss
                b = args.cost_limit-ep_costs
                loss_pi_c = torch.nn.functional.relu(Ac - min(b, b*config["w"]))

                loss_pi = loss_pi_r + kappa*loss_pi_c + config["entropy"]*policy.actor(obs_b).entropy().mean()
            ...
            kl = torch.distributions.kl.kl_divergence(old_distribution, new_distribution).sum(-1, keepdim=True).mean().item()
            final_kl = kl
            update_counts += 1
            #print(final_kl, loss_kl)
        ...
        if not logger.logged:
            ...
            logger.log_tabular("Train/Kappa", kappa)
